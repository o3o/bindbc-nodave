...
extern(C) @nogc nothrow {
   ...
   alias p_daveConstructUpload = void function(PDU* p, char blockType, int blockNr);
   alias p_daveConstructDoUpload = void function(PDU* p, int uploadID);
   alias p_daveConstructEndUpload = void function(PDU* p, int uploadID);
   ...
   alias pdaveAddBitVarToWriteRequest = void function(PDU* p, int area, int DBnum, int start,
    ...
    */
   alias pdaveNewInterface = daveInterface* function(daveOSserialType nfd, const(char)* nname,
...
p_daveConstructUpload        _daveConstructUpload        ;
p_daveConstructDoUpload      _daveConstructDoUpload      ;
p_daveConstructEndUpload     _daveConstructEndUpload     ;
...
NodaveSupport loadNodave()
{
   version(Windows) {
      ...
      const(char)[][2] libNames = [
         "libnodave.so",
         "/usr/local/lib/libnodave.so",
      ];
   }
   else static assert(0, "bindbc-nodave is not yet supported on this platform.");

   NodaveSupport ret;
   foreach(name; libNames) {
      ret = loadNodave(name.ptr);
      if(ret != NodaveSupport.noLibrary) break;
   }
   return ret;
}
...
NodaveSupport loadNodave(const(char)* libName)
{
   ...
   //lib.bindSymbol(cast(void**)&nodaveInit,"nodaveInit");
...
lib.bindSymbol(cast(void**)&_daveConstructUpload        , "_daveConstructUpload"       ) ;
lib.bindSymbol(cast(void**)&_daveConstructDoUpload      , "_daveConstructDoUpload"     ) ;
lib.bindSymbol(cast(void**)&_daveConstructEndUpload     , "_daveConstructEndUpload"    ) ;
